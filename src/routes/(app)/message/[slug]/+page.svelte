<script lang="ts">
  ...
  import BreadPopup from "$lib/component/BreadPopup.svelte";
  ...
  {#if $user?.uid === data.uid}
    <form
      ...
      on:submit|preventDefault={async () => {
        (await onSendMessage()) ? onToggle() : null;
      }}
      class="flex flex-col justify-center my-6 max-w-[32rem] mx-auto"
    >
      ...
      {/if}
      <div class="mx-auto">
        <PrimaryButton type="submit" class="px-4 py-2 min-w-[16rem] my-4"
          >ส่ง</PrimaryButton
        >
      </div>
    </form>
